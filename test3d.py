...
def galvo(a, b):
    ...
    # Define y mirror
    X, Y = np.meshgrid([-dim / 4, dim / 4], [-dim / 8, dim / 8])
    Z = (Y * (mechthetaY/100 + .45))

    # Define x mirror
    X2, Y2 = np.meshgrid([-dim / 8, dim / 8], [-dim / 4, dim / 4])
    Z2 = (X2 * (mechthetaX/100 + .45))

    # Plot y mirror.
    sf = ax.plot_surface(X, Y + .5, Z + .5, color='gray', alpha=.5, linewidth=0, zorder=1)  # y mirror
    # Plot x mirror
    sf1 = ax.plot_surface(X2, Y2 + .5, Z2 - .5, color='blue', alpha=.5, linewidth=0, zorder=3)  # x mirror
